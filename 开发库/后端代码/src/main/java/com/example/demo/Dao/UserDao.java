...
public interface UserDao {
    ...
    List<User> getUser(String id,String word);      //登录验证
	
	@Insert("INSERT into zhuce(id,word,name,sex,phone,email,city,account) "   //注册用户信息插入待验证用户表
			+ "values(#{0},#{1},#{2},#{3},#{4},#{5},#{6},#{7}) ")
	void insertRegisteredUser(String id,String word,String name,String sex,String phone,String email,String city,String account);    
	
	@Select("SELECT * FROM zhuce where id = #{0}")
    List<User> getRegisteredUser(String id);      //验证用户注册是否重复提交
	
	@Select("SELECT * FROM zhuce ")
    List<User> getUnauditedUser();      //获取待验证的注册用户列表
	...
	void updateUserInfo(String name,String phone,String email,String city,String account,String id,String word);
	
	@Update("UPDATE user SET licence = #{2},card = #{3},status = #{4} where id = #{0} and word = #{1}" )  //用户注册商家
	void shopRegister(String id,String word,String licence,String card,String status);
	
	@Select("SELECT id,word,name,sex,phone,email,city,account,licence,card from user where status = #{0} " ) //获取所有注册商家的用户
	List<User> getUnauditedShop(String status);
	...
	void allowShopApply(String status,String id);
	...
	List<User> userMoney(String id);
	
	@Update("UPDATE user SET grade = #{1} where id = #{0}" )  //更新用户积分
	void userGrade(String id,String grade);
	...
	void rechargeWallet(String id,String money); //充值
}
