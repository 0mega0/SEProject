            ...
            @row-click="openDetails" border>
                ...
                <el-table-column prop="status" label="商品状态" width="80" >
                    <template slot-scope="scope">
                        <p v-if="scope.row.status === '4'" >待收货</p>
                    </template>
                </el-table-column>
                ...
                <el-table-column  width="90" label="操作">
                    <template slot-scope="scope">
                        <el-button size="mini" type="primary" @click.native.prevent="deleteRow(scope.$index, tableData)">收货</el-button>
                    </template>
                </el-table-column>
            </el-table>
        </el-main>
...
export default {
  ...
  methods: {
    ...
    getData () { // 后台获取数据
      ...
      this.$axios.post('http://localhost:8080/shouhuo', formData, config)
        .then(res => {
    ...
    sure () {
      ...
      formData.append('id', sessionStorage.getItem('userid'))
      formData.append('word', sessionStorage.getItem('userword'))
      formData.append('shopid', this.column.shopid)
      ...
      this.$axios.post('http://localhost:8080/ushou', formData, config)
        .then(res => {
          this.getData()
          this.$notify({
            title: '成功',
