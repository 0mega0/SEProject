      ...
      <el-container style="height: 700px; border: 1px solid #eee">
        ...
        <el-main style="height:0;flex-grow:1;">
            ...
            @row-click="openDetails" border>
                ...
                <el-table-column prop="account" label="银行账户" width="200" >
                </el-table-column>
                ...
                <el-table-column  width="160" label="操作">
                    <template slot-scope="scope">
                        <el-button size="mini" type="primary" @click.native.prevent="deleteRow(scope.$index, tableData)">允许注册</el-button>
                    </template>
                </el-table-column>
            </el-table>
        </el-main>
...
export default {
  ...
  methods: {
    ...
    getData () { // 后台获取数据
      ...
      this.$axios.post('http://localhost:8080/getshopzhuce', formData, config)
        .then(res => {
    ...
    sure () { // 确认允许注册，更改用户status
      ...
      this.$axios.post('http://localhost:8080/allowshopzhuce', formData, config)
        .then(res => {
          this.$notify({
            title: '成功',
            message: '操作成功',
